<section class="space-y-6">
    ...
    <form method="post" action="{{ route('profile.saveCurso') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            <x-text-input id="instituicao" name="instituicao" type="text" class="mt-1 block w-full"  required autofocus autocomplete="instituicao" readonly/>
            <x-input-error class="mt-2" :messages="$errors->get('instituicao')" />
        </div>
        <div>
            <x-input-label for="numero_aluno" :value="__('Número de Aluno')" />
            <x-text-input id="numero_aluno" name="numero_aluno" type="text" class="mt-1 block w-full"  required autofocus autocomplete="numero_aluno" readonly/>
